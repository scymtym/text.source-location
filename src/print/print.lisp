...
(defun print-annotated-lines (stream locations &optional colon? at? &key line-number-width (context-lines 2) (context-columns 16))
  (declare (ignore colon? at?))
  (let+ (  ; TODO should be parameters
...
(defun print-annotations (stream annotations &key (context-lines 2))
  (let* ((clusters (cluster-locations
                    annotations
                    :key                     #'location
                    :intra-cluster-gap-limit (+ 2 (* 2 context-lines))))) ; TODO the following should hold :intra-cluster-gap-limit >= context-lines 
